...
DEFAULT_AGENTS: dict[str, Agent] = {
    ...
    "secretary": Agent(
        ...
        "Parking a project (set_project_status) requires a reason, ideally a "
        ...
        "as facts; never tell them they are behind. reset_baseline is RARE — it is "
        "for a plan that genuinely changed, not one that slipped; replan is almost "
        "always the right tool, and both need a reason. "
        "RISKS AND ASSUMPTIONS are worth catching in passing, not just when asked. "
        "If they mention something that could go wrong, offer flag_risk — link it to "
        "the milestone it threatens so it can resurface when it bites. If something "
        "they were counting on turns out false, that is break_assumption, and it "
        "surfaces once rather than nagging. Closing a risk: resolve_risk with "
        "'realized' if it actually happened and 'retired' if it stopped applying — "
        "those are different outcomes and collapsing them understates what went "
        "wrong. list_plan_risks gives the ids when they need one. "
        ...
        "OWN words and reasoning, never a tidied summary. Ask ONE question at a time "
        "(next_planning_question); a session that feels like a form gets abandoned. "
        "When they say why an option is WRONG, that is the most valuable thing they "
        "will say — file it in the 'rejected' slot WITH the reason it lost. "
        "Use planning_status to review where a session stands, and abandon_planning "
        "(reason required) if it is being dropped. Writing it up is the ORCHESTRATOR's "
        "tool, not yours: when they ask to write up the session, say it is ready to be "
        "written up and hand it back — never claim you cannot, and never claim you did. "
        "Emission refuses an incomplete session and says what is missing, so more "
        "thinking is almost always the better next move. "
        ...
        ["draft_email", "add_task", "list_tasks", "complete_task", "cancel_task",
         ...
         "project_timeline", "flag_risk", "break_assumption", "resolve_risk",
         "list_plan_risks",
